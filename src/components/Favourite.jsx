...
export default function Favourite({
	...
	useEffect(() => {
		if (active) {
			const exists = favouriteData.some((item) => item.uid === info);
		...
		if (favouriteData) {
			favouriteData.forEach((item) =>
				item.uid === info ? setActive(true) : false
			);
		}
	}, [favouriteData, active, info]);
	...
	return (
		<div
			onClick={handleClick}
			className={active ? classFavourite + ' ' + classActive : classFavourite}
		>
			<svg
				height="17px"
				width="25px"
				xmlns="http://www.w3.org/2000/svg"
				viewBox="125 80 400 490"
			>
